...
public:
    ...
    core::u8_view name() const noexcept {
        return qualified_.substr(prefix_length_ ? prefix_length_ + 1 : 0);
    }

    /// The prefix, empty when the name carries none.
    core::u8_view prefix() const noexcept { return qualified_.substr(0, prefix_length_); }
    ...
    core::u8_view namespace_uri() const noexcept { return namespace_uri_; }

    /// The name as the document wrote it, prefix included.
    core::u8_view qualified() const noexcept { return qualified_; }
    ...
    bool is(std::string_view uri, std::string_view local) const noexcept {
...
public:
    ...
    core::u8_view name() const noexcept { return name_.name(); }
    core::u8_view prefix() const noexcept { return name_.prefix(); }
    core::u8_view namespace_uri() const noexcept { return name_.namespace_uri(); }
    core::u8_view qualified_name() const noexcept { return name_.qualified(); }
...
public:
    node(node_type type, qualified_name name, int line, int column) noexcept
        : name_(name), type_(type), line_(line), column_(column) {}

    node_type type() const noexcept { return type_; }
    bool is_element() const noexcept { return type_ == node_type::element; }
    bool is_text() const noexcept { return type_ == node_type::text; }
    ...
    core::u8_view namespace_uri() const noexcept { return name_.namespace_uri(); }
    core::u8_view qualified_name() const noexcept { return name_.qualified(); }
    const xml::qualified_name& full_name() const noexcept { return name_; }
    ...
    const node* operator()(std::string_view name, std::size_t index = 0) const noexcept {
    ...
    bool has_attribute(std::string_view name) const noexcept { return attribute(name).has_value(); }
...
private:
    ...
    void set_value(core::u8_view text) noexcept { value_ = text; }
    void set_attributes(const attribute_span list) noexcept { attributes_ = list; }
...
public:
    ...
    public:
        ...
        iterator(node_list::const_iterator at, node_list::const_iterator end, std::string_view name) noexcept
            : at_(at), end_(end), name_(name) {
            skip_to_match();
        }

        const node& operator*() const noexcept { return *at_; }
        const node* operator->() const noexcept { return &*at_; }
    ...
    named_range(const node_list& children, std::string_view name) noexcept
        : children_(&children), name_(name) {}

    iterator begin() const noexcept { return iterator(children_->begin(), children_->end(), name_); }
    iterator end() const noexcept { return iterator(children_->end(), children_->end(), name_); }
...
public:
    ...
    public:
        ...
        iterator() noexcept = default;
        iterator(const node* root, const node* at) noexcept : root_(root), at_(at) { skip_to_text(); }

        core::u8_view operator*() const noexcept { return at_->value(); }
    ...
    explicit text_range(const node& root) noexcept : root_(&root) {}

    iterator begin() const noexcept { return iterator(root_, root_); }
    iterator end() const noexcept { return iterator(root_, nullptr); }
