...
сопряжённых корней. Корни — `std::complex<double>`, и печатает их тот же
`format(u"{:.9g}", root)`, что и дискриминант: девять цифр у каждой части,
`(-1+2i)`; вещественный корень выглядит как `(1.64575131+0i)`.
