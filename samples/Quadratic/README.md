...
и число в них кладёт сам `NumberBox`; пустое поле у него — NaN, и такой
коэффициент модель считает не введённым.
...
`value = BindInput{eq.a}` у поля ввода, `text = BindOutput{eq.D}` у поля
вывода — и больше ничего. Направление названо в самой привязке: коэффициент
идёт из `NumberBox` в модель и только туда, модель поле никогда не
переписывает; ответ идёт из модели в `TextBox` и только туда, а поля ответов
объявлены `isReadOnly` и только показывают. Двусторонний `Bind{}`, который
берёт направление из природы свойства, здесь не нужен ни одному полю.
