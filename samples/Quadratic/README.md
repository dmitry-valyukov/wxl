...
приложение — двести строк одного `main.cpp`, из них интерфейс одно выражение.
Обработчиков ввода нет ни одного: правка любого коэффициента сама доходит до
корней.

Что показано: реактивная модель на `observable`, привязки `Bind{}` вместо
обработчиков, вложенные пресеты, плейсхолдеры вместо подписей, картинка фона
на сцене `CompositionWindow` и счёт корней без потери точности.
...
захватывает в обработчике, который wxl зовёт на пути вниз.
...
двусторонняя; поля ответов объявлены `isReadOnly` и только показывают. Ни
один контрол не лежит в переменной — всё, ради чего его держали бы, написано
в его же скобках, включая фокус в первом поле через `onLoaded`.
...
вкладываются друг в друга, поэтому «крупно и по центру» написано один раз.
...
меняет размер синхронно в `WM_SIZE`, оттого держится за рамку без отставания.
...
Декодирует и держит картинку кэш текстур Win2D, который окно заводит само.
