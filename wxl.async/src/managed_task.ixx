export module wxl.async:managed_task;
...
/// A coroutine somebody manages: it is handed back to its caller to be held,
/// swept and read.
///
/// That is the whole difference from `task`, which owns itself and answers to
/// nobody. Here the caller keeps what comes back -- the Reader's `Io` keeps
/// them in a vector and sweeps the finished ones -- because there is something
/// to read at the end, and, while an operation of this module is in flight,
/// because there is something the worker still points at.
///
...
/// the awaitable's business, not this one's. So "managed" is about ownership,
/// not about where the work goes.
///
...
/// exception from. The frame is destroyed by the managed_task, and what that
