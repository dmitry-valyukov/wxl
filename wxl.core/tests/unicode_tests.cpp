...
TEST(unicode, russian_round_trip) {
    ...
    EXPECT_EQ(wide.size(), wxl::core::unicode::code_point_count(assume_valid(russian)));
}

TEST(unicode, surrogate_pair_round_trip) {
    const std::string smile = "\xF0\x9F\x98\x80";  // U+1F600

    EXPECT_EQ(wxl::core::unicode::code_point_count(assume_valid(smile)), 1u);
    EXPECT_EQ(wxl::core::unicode::utf16_size(assume_valid(smile)), 2u);

    const wxl::core::u16_text wide = assume_valid(smile).to_utf16();

    ASSERT_EQ(wide.size(), 2u);
    EXPECT_TRUE(wxl::core::unicode::is_high_surrogate(wide.wchars()[0]));
    EXPECT_TRUE(wxl::core::unicode::is_low_surrogate(wide.wchars()[1]));
...
TEST(unicode, round_trip_at_every_length) {
    ...
    for (const char32_t code_point : {U'a', U'б', U'字', U'\U0001F600'})
        for (int repeat = 0; repeat != 4; ++repeat) wxl::core::unicode::append_utf8(text, code_point);

    for (std::size_t size = 0; size != text.size(); ++size) {
        if (size != text.size() && wxl::core::unicode::is_continuation(text[size])) continue;

        const std::string_view prefix = std::string_view(text).substr(0, size);

        EXPECT_TRUE(wxl::core::unicode::is_valid_utf8(prefix)) << "at " << size;
...
TEST(unicode, sizes_are_what_the_conversion_writes) {
    for (const std::string_view text : {ascii, russian, mixed}) {
        ...
        EXPECT_EQ(wide.size(), wxl::core::unicode::utf16_size(assume_valid(text)));
        EXPECT_EQ(text.size(), wxl::core::unicode::utf8_size(wide));
    }
}

TEST(unicode, code_point_sizes) {
    EXPECT_EQ(wxl::core::unicode::utf8_size(U'a'), 1);
    EXPECT_EQ(wxl::core::unicode::utf8_size(U'б'), 2);
    EXPECT_EQ(wxl::core::unicode::utf8_size(U'字'), 3);
    EXPECT_EQ(wxl::core::unicode::utf8_size(U'\U0001F600'), 4);

    EXPECT_EQ(wxl::core::unicode::utf16_size(U'字'), 1);
    EXPECT_EQ(wxl::core::unicode::utf16_size(U'\U0001F600'), 2);
}

TEST(unicode, counting_characters_not_bytes) {
    EXPECT_EQ(wxl::core::unicode::code_point_count(assume_valid(""sv)), 0u);
    EXPECT_EQ(wxl::core::unicode::code_point_count(assume_valid(ascii)), ascii.size());
    EXPECT_EQ(wxl::core::unicode::code_point_count(assume_valid("привет"sv)), 6u);
    EXPECT_EQ(wxl::core::unicode::code_point_count(assume_valid("a😀b"sv)), 3u);
}

TEST(unicode, valid_utf8_is_accepted) {
    for (const std::string_view text : {""sv, ascii, russian, mixed})
        EXPECT_FALSE(wxl::core::unicode::find_invalid_utf8(text).has_value());
}
...
TEST(unicode, broken_utf8_is_named_by_its_offset) {
    ...
    for (const auto& one : cases) {
        const auto found = wxl::core::unicode::find_invalid_utf8(one.text);
...
TEST(unicode, the_smallest_and_the_largest) {
    std::string text;
    wxl::core::unicode::append_utf8(text, U'\0');
    wxl::core::unicode::append_utf8(text, static_cast<char32_t>(wxl::core::unicode::max_code_point));

    EXPECT_TRUE(wxl::core::unicode::is_valid_utf8(text));
    EXPECT_EQ(wxl::core::unicode::code_point_count(assume_valid(text)), 2u);
...
TEST(unicode, broken_utf16_is_named_by_its_offset) {
    ...
    EXPECT_EQ(wxl::core::unicode::find_invalid_utf16(lone_high), wxl::core::nullable<std::size_t>(2));
    EXPECT_EQ(wxl::core::unicode::find_invalid_utf16(lone_low), wxl::core::nullable<std::size_t>(2));
    EXPECT_EQ(wxl::core::unicode::find_invalid_utf16(high_then_letter), wxl::core::nullable<std::size_t>(2));

    EXPECT_TRUE(wxl::core::unicode::is_valid_utf16(L"ab\xD83D\xDE00z"));
}
...
TEST(unicode, a_cut_steps_back_off_the_second_half_of_a_pair) {
    ...
    EXPECT_EQ(wxl::core::unicode::floor_code_point_boundary(text, 0), 0u);
    EXPECT_EQ(wxl::core::unicode::floor_code_point_boundary(text, 1), 1u);
    EXPECT_EQ(wxl::core::unicode::floor_code_point_boundary(text, 2), 1u);
    EXPECT_EQ(wxl::core::unicode::floor_code_point_boundary(text, 3), 3u);
    EXPECT_EQ(wxl::core::unicode::floor_code_point_boundary(text, 4), 4u);
    EXPECT_EQ(wxl::core::unicode::floor_code_point_boundary(text, 100), 4u);

    // A pair at the very start has nothing before it to be confused with.
    EXPECT_EQ(wxl::core::unicode::floor_code_point_boundary(u"\xD83D\xDE00", 1), 0u);
}
...
TEST(unicode, a_cut_beside_a_lone_surrogate_stays_put) {
    ...
    EXPECT_EQ(wxl::core::unicode::floor_code_point_boundary(lone_low, 1), 1u);
    EXPECT_EQ(wxl::core::unicode::floor_code_point_boundary(low_then_high, 1), 1u);
}

// Constant evaluation answers the same, so a limit known at compile time can
// be cut at compile time too.
static_assert(wxl::core::unicode::floor_code_point_boundary(u"a\xD83D\xDE00z", 2) == 1);
...
TEST(unicode, walking_hands_out_position_and_bytes) {
    ...
    const wxl::core::unicode::code_points walk(assume_valid(text));
...
TEST(unicode, walking_works_in_a_range_for) {
    ...
    for (const char32_t code_point : wxl::core::unicode::code_points(assume_valid(russian))) {
        EXPECT_TRUE(wxl::core::unicode::is_scalar_value(code_point));
        ++count;
    }

    EXPECT_EQ(count, wxl::core::unicode::code_point_count(assume_valid(russian)));
}

TEST(unicode, appending_grows_what_is_already_there) {
    std::wstring wide = L"<<";
    wxl::core::unicode::append_utf16(wide, assume_valid(russian));
    wxl::core::unicode::append_utf16(wide, U'>');
    ...
    std::string narrow = "<<";
    wxl::core::unicode::append_utf8(narrow, wide_russian);
