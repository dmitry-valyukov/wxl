...
public:
    ...
    inline ~semaphore() { ::CloseHandle(handle_); }

    /// Increments the count by one, waking a single waiter if any.
    inline void release() { ::ReleaseSemaphore(handle_, 1, nullptr); }

    /// Blocks until the semaphore is signaled.
    inline void acquire() { ::WaitForSingleObject(handle_, INFINITE); }
    ...
    inline bool try_acquire() { return ::WaitForSingleObject(handle_, 0) == WAIT_OBJECT_0; }
