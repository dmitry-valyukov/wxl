...
try {
    foreach ($raw in $Do -split ';') {
        ...
        switch ($parts[0]) {
            ...
            'ctrlwheel' { MoveTo ([int]$parts[1]) ([int]$parts[2]); [Probe.Native]::keybd_event(0x11, 0, 0, [IntPtr]::Zero); Start-Sleep -Milliseconds 50; [Probe.Native]::mouse_event(0x0800, 0, 0, [BitConverter]::ToUInt32([BitConverter]::GetBytes([int]$parts[3]), 0), [IntPtr]::Zero); Start-Sleep -Milliseconds 50; [Probe.Native]::keybd_event(0x11, 0, 2, [IntPtr]::Zero) }
